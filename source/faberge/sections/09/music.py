...
def va(m):
    with baca.scope(m[1]) as o:
        for plt in baca.select.plts(o):
            ...
            if duration == abjad.ValueDuration(1, 2):
                baca.quadruple_staccato(plt)
            elif duration == abjad.ValueDuration(1, 3):
    ...
    with baca.scope(m[2]) as o:
        for plt in baca.select.plts(o):
            duration = abjad.get.duration(plt)
            if duration == abjad.ValueDuration(5, 12):
                baca.quadruple_staccato(plt)
            elif duration == abjad.ValueDuration(5, 18):
                baca.stem_tremolo(plt)
        baca.hairpin(
            baca.select.clparts(o, [1]),
...
def vc(m):
    ...
    with baca.scope(m.get(1, 3)) as o:
        ...
        baca.hairpin(
            o,
            "p<f-poco-scratch",
            # baca.tweak.parent_alignment_x(-1, i=-1),
            # baca.tweak.self_alignment_x(-1, i=-1),
            rleak=True,
        )
    for n in [5, 6, 7]:
        with baca.scope(m[n]) as o:
...
def make_layout(environment):
    ...
    spacing = baca.layout.Spacing(
        default=(1, 16),
        strict_overrides=(baca.layout.Override((1, 4), (1, 12)),),
    )
